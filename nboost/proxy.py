...
class Proxy(SocketServer):
    ...
    def __init__(self, data_dir: Path = PKG_PATH.joinpath('.cache'),
        ...
        self.cids_path = _config['cids_path']
        self.cvalues_path = _config['cvalues_path']
        ...
        self.status = {
            ...
            'paths': {
                ...
                'cids': _config['cids_path'],
                'cvalues': _config['cvalues_path']
            },
    ...
    def get_topk(self, request: dict, topk_path: str, default_topk: str) -> int:
        topks = get_jsonpath(request, topk_path)
        return int(topks[0]) if topks else default_topk

    def get_query(self, request: dict, query_path: str) -> str:
        queries = get_jsonpath(request, query_path)
    ...
    def get_cids(self, response: dict, cids_path: str) -> list:
        cids = get_jsonpath(response, cids_path)
        return flatten(cids)
    ...
    def get_cvalues(self, response: dict, cvalues_path: str) -> list:
        return get_jsonpath(response, cvalues_path)

    def get_choices(self, response: dict, choices_path: str) -> list:
        choices = get_jsonpath(response, choices_path)
    ...
    def loop(self, client_socket: socket.socket, address: Tuple[str, str]):
        ...
        try:
            ...
            runtime_configs = request['body'].pop('nboost', {})

            query_path = runtime_configs.get('query_path', self.query_path)
            choices_path = runtime_configs.get('choices_path', self.choices_path)
            cids_path = choices_path + '.[*].' + runtime_configs.get('cids_path', self.cids_path)
            cvalues_path = choices_path + '.[*].' + runtime_configs.get('cvalues_path', self.cvalues_path)
            topk_path = runtime_configs.get('topk_path', self.topk_path)
            default_topk = runtime_configs.get('default_topk', self.default_topk)

            topk = self.get_topk(request, topk_path, default_topk)
            query = self.get_query(request, query_path)
            ...
            if self.rerank:
                choices = self.get_choices(response, choices_path)
                cids = self.get_cids(response, cids_path)
                cvalues = self.get_cvalues(response, cvalues_path)
            ...
            if self.qa:
                ...
                if cvalues:
                    start_time = time.perf_counter()
                    answer, start_pos, stop_pos, score = self.qa_model.get_answer(query, cvalues[0])
                    total_time = time.perf_counter() - start_time
                    self.update_averages(qa_time=total_time)

                    if score > self.qa_threshold:
                        response['body']['nboost']['answerStartPos'] = start_pos
                        response['body']['nboost']['answerStopPos'] = stop_pos

                        if qa_cids and cids and cids[0] in qa_cids.keys():
                            qa_start_pos, qa_end_pos = qa_cids[cids[0]]
                            overlap = calculate_overlap(qa_start_pos,qa_end_pos, start_pos, stop_pos)
